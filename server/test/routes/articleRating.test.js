...
describe('Article rating', () => {
    ...
    chai.request(server)
      ...
      .send({
        user: {
          firstName: 'myfitstname',
          lastName: 'mylastname',
          username: 'falseupdate',
          password: 'R!vendell12',
          email: 'falseupdate@gmail.com',
        },
      })
      .end((err, res) => {
        Object.assign(userFalseUpdate, res.body);
        done();
      });
  });
  ...
  before((done) => {
    chai.request(server)
      .get(`/api/users/verify/${userFalseUpdate.user.token}`)
      .end((err, res) => {
        done();
      });
  });
    ...
    it('The authors should not be able to rate their articles', (done) => {
      chai.request(server)
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.have.property('errors');
          expect(res.body.errors).to.have.property('message');
          expect(res.body.errors.message).to.equal('You can\'t provide ratings for your article');
          done();
        });
    });
    ...
    it('should not rate an article if no rating value is provided', (done) => {
      chai.request(server)
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.have.property('errors');
          expect(res.body.errors).to.have.property('message');
          expect(res.body.errors.message).to.equal('Invalid rating value');
          done();
        });
    });
    ...
    it('it should return the rating for the article with articleId', (done) => {
      chai.request(server)
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.have.property('ratingDetails');
          expect(res.body.ratingDetails.ratingCount).to.equal('2');
          expect(res.body.ratingDetails.averageRating).to.equal('4.00');
          done();
        });
    });
    ...
    it('users should be able to update their ratings', (done) => {
      chai.request(server)
        ...
        .end((err, res) => {
          ...
          expect(res.body.ratingDetails.averageRating).to.equal('5.00');
          done();
        });
    });
    ...
    it('should not update if user has not previously rated', (done) => {
      chai.request(server)
        ...
        .end((err, res) => {
          ...
          expect(res.body).to.have.property('errors');
          expect(res.body.errors).to.have.property('message');
          expect(res.body.errors.message).to.equal('Rating not found, no update was made');
          done();
        });
    });
